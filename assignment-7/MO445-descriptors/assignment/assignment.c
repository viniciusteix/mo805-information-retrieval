...
/*
	INPUT:
		./assignment "path_images_pgm" number_of_images "results.csv"

	OUTPUT:
		results.csv with all images names closeded of each query (image).
*/

int main(int argc,char **argv){
	
	int size, i = 0, j;
	int fixedlen = strtol(argv[2],NULL,10);

	/* Estrutura para ler todas as imagens do diretorio imgs/ */
	char dir_name[1024], result_name[1024];
	strcpy(dir_name,argv[1]);
	strcpy(result_name,argv[3]);
	DIR *d;
	struct dirent *dir;
	d = opendir(dir_name);
